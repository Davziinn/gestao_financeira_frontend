...
import { DialogEditarTransacao } from "./Dialogs/DialogEditarTransacao";
import type { ITransacao } from "../../../contexts/TransactionContext";
import { DialogDeletarTransacao } from "./Dialogs/DialogDeletarTransacao";
    ...
    >
      ...
      >
        <Table sx={{ width: "100%", tableLayout: "fixed" }}>
          <TableHead>
            <TableRow>
              {["Data", "Descrição", "Categoria", "Valor", "Tipo", "Ações"].map(
                (col) => (
                  <TableCell
                    key={col}
                    sx={{
          ...
          <TableBody>
            {transacoes.map((row, index) => (
              <TableRow key={index}>
                ...
                <TableCell>
                  <IconButton onClick={() => handleOpenDialogEditar(row)}>
                    <EditIcon sx={{ color: "white" }} />
                  </IconButton>
                  <IconButton onClick={() => handleOpenDialogDeletar(row)}>
